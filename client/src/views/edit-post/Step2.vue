...
import StageModal from '@/components/post-create-edit/StageModal.vue';
...
const openGallery = function(stageImages: File[] | string[]){
    ...
    if(typeof(stageImages[0]) === 'string'){
        galleryImages.value = stageImages as string[];
    }else{
        galleryImages.value.forEach(url => URL.revokeObjectURL(url));
        galleryImages.value = stageImages.map(file => URL.createObjectURL(file as File));
    }
